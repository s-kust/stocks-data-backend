...
def get_failed_imports_list():
    ...
    if failed_imports_file_fresh:
        ...
        if isinstance(failed_imports_internal_list, list):
            ...
            print("failed_imports list downloaded from S3, but not list, so now reset it as empty list")
...
def process_fx_row(event_internal, failed_imports_internal):
    ...
    if combined_ticker_fx in failed_imports_internal:
        ...
        response_dict = {'import_success': False, 'failed_ticker': combined_ticker_fx, 'note': event_internal['Note'], 'api_call_count': 0}
    ...
    if fresh_ticker_file_found:
        ...
        response_dict = _prepare_fx_success_response_dict(event_internal, combined_ticker_fx_file_name, 0)
        return response_dict
            
    print ('Fresh ticker data file NOT found in our S3, so now import')
    fx_row_import_result = _import_forex_row(event_internal, combined_ticker_fx_file_name)        
    if not fx_row_import_result:
        _failed_imports_new_append_upload(combined_ticker_fx, failed_imports_internal)
        response_dict = {'import_success': False, 'failed_ticker': combined_ticker_fx, 'note': event_internal['Note'], 'api_call_count': 1}
    else:
        response_dict = _prepare_fx_success_response_dict(event_internal, combined_ticker_fx_file_name, 1)
    return response_dict
...
def process_stocks_row(event_internal, failed_imports_internal):
    ...
    process_result, ticker_1_file_name, api_call_count_1 = process_single_stock_ticker(event_internal['Ticker1'], failed_imports_internal)
    if not process_result:
        response_dict = _prepare_stock_fail_response_dict(event_internal['Ticker1'], event_internal['Note'], api_call_count_1)
        return response_dict
    if event_internal['Ticker2'] == '':
        ...
        return response_dict
    print('row - Stocks_ETFs, relative two tickers', event_internal['Ticker1'], "-", event_internal['Ticker2'])    
    process_result, ticker_2_file_name, api_call_count_2 = process_single_stock_ticker(event_internal['Ticker2'], failed_imports_internal)
    api_call_count_total = api_call_count_1 + api_call_count_2
    if not process_result:
        response_dict = _prepare_stock_fail_response_dict(event_internal['Ticker2'], event_internal['Note'], api_call_count_total)
        return response_dict
    response_dict = {
        'import_success': True, 
